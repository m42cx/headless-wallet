...
function readDestinationAddress(wallet, handleAddress) {
    db.query('SELECT address FROM my_addresses WHERE wallet=? ORDER BY is_change DESC, address_index ASC LIMIT 1', [wallet], (rows) => {
        if (rows.length === 0) { throw Error('no dest address'); }
...
function consolidate(wallet, signer) {
    ...
    mutex.lock(['consolidate'], (unlock) => {
        determineCountOfOutputs(asset, wallet, (count) => {
            ...
            if (count <= conf.MAX_UNSPENT_OUTPUTS) { return unlock(); }
            const count_to_spend = Math.min(count - conf.MAX_UNSPENT_OUTPUTS + 1, constants.MAX_INPUTS_PER_PAYMENT_MESSAGE - 1);
                ...
                db.query(
                    ...
                    (rows) => {
                        ...
                        function addLargeInputIfNecessary(onDone) {
                            let target_amount = 1000 + TRANSFER_INPUT_SIZE * rows.length + AUTHOR_SIZE * arrAddresses.length;
                            if (input_amount > target_amount) { return onDone(); }
                            ...
                            db.query(
                                ...
                                (large_rows) => {
                                    if (large_rows.length === 0) { return onDone('no large input found'); }
                                    const row = large_rows[0];
                        ...
                        addLargeInputIfNecessary((err) => {
                            ...
                            const arrUsedAddresses = Object.keys(assocUsedAddresses);
